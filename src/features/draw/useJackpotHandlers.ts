...
export function useJackpotHandlers(
  ...
  useEffect(() => {
    ...
    if (cycleIndex === prevCycleIndex.current + 1) {
      ...
      const hadValidEntry = userNumbers.length === 6;
      ...
      console.log(`[useJackpotHandlers] hadValidEntry=${hadValidEntry}, incremented already? ${incrementedCycles.current.has(prevCycle)}`);

      // Only increment jackpot ONCE per prevCycle
      if (hadValidEntry && !incrementedCycles.current.has(prevCycle)) {
        ...
        console.log(`[useJackpotHandlers] No valid entry for prevCycle ${prevCycle}. Jackpot not incremented.`);
